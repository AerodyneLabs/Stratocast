				...
				if(pred.direction === 'reverse') {
					...
					pred.features[0] = {
						...
						geometry: {
							...
							coordinates: [pred.loc[0], pred.loc[1], pred.startAlt]
						},
						properties: {}
					};
					pred.features[1] = {
					...
					while (radius <= pred.balloon.burstRadius) {
						...
						pred.features[1].geometry.coordinates.push([point[0], point[1], alt])
						obs = prof.getObservationAtAltitude(alt);
						radius = pred.balloon.getRadius(obs);
					}
					...
					while (alt >= pred.startAlt) {
						...
						pred.features[3].geometry.coordinates.push([point[0], point[1], alt]);
					}
