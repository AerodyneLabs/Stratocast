...
var redis = redisLib.createClient(redisUrl.port, redisUrl.hostname);
redis.auth(redisUrl.password);

var UPDATE_INTERVAL = 6 * 60 * 60 * 1000; // ms between source updates
var UPDATE_OFFSET = 4 * 60 * 60 * 1000; // ms delay before starting update
...
var MongoUrl = process.env.MONGODB_URI || 'mongodb://localhost:27017/stratocast';
...
var dataUrl = function(analysisTimestamp, forecastHours) {
  ...
  // Example file name
  // gfs.t18z.pgrb2b.1p00.f384
  var file = sprintf(
    'gfs.t%02dz.pgrb2b.1p00.f%03d', [
      analysisTime.getUTCHours(),
      forecastHours
    ]);
...
putCache = function(time, lat, lng, sounding, next) {
    ...
    MongoClient.connect(MongoUrl, function(err, db) {
        ...
        db.collection('soundingCache').update({time: time, loc:[lng, lat]},
                { $set: { sounding } },
                {upsert: true},
                function(err, res) {
                    next(err);
...
getCache = function(time, lat, lng, next) {
    ...
    MongoClient.connect(MongoUrl, function(err, db) {
        ...
        db.collection('soundingCache').findOne({
            time: time,
            loc:[lng, lat]
            },
            function(err, doc){
...
clearCache = function(time, next) {
    ...
    MongoClient.connect(MongoUrl, function(err, db) {
        ...
        if(err) {
            console.log("MongoDB error in clearCache: " + err);
        } else {
            db.collection('soundingCache').remove({
                time: time
            });
 ...
 *
 * Passes any error as the first parameter to the callback.
 * The sounding object is passed as the second parameter if successful.
 * 
...
extractSounding = function(time, lat, lng, next) {
  ...
  getFilename(time, function(err, fileName) {
    if(err) {
      ...
      var parsed = [];
      var parser = spawn('grib2/wgrib2/wgrib2', [
        fileName,
        '-ij',
        i,
        j
...
getFilename = function(time, next) {
    ...
    MongoClient.connect(MongoUrl, function(err, db) {
        ...
        db.collection('soundingFiles').findOne({time: time}, function(localerr, res) {
            if(localerr || !res) {
                next(localerr);
            } else {
                next(null, res.record.fileName);
            }
        });
...
setFilename = function(time, fileName, next) {
  ...
  MongoClient.connect(MongoUrl, function(err, db) {
        ...
        db.collection('soundingFiles').update({
            time: time
        },
        {
            $set: { record }
...
/**
 * Get a sounding from the cache or from the file store.
 * 
 * @param  {Date}   time UTC time of the sounding
 * @param  {Number}   lat  Latitude of the sounding
 * @param  {Number}   lng  Longitude of the sounding
...
exports.getSounding = function(time, lat, lng, next) {
  // Round inputs to nearest expected value
  var nearest = Math.round(time.getTime() / UPDATE_INTERVAL) * UPDATE_INTERVAL;
  time = new Date(nearest);
  lat = Math.round(lat);
  lng = Math.round(lng);
  if(lng < 0) lng += 360;
  
  // Check cache
  getCache(time, lat, lng, function(err, res) {
    if(!err && res) { // Cache hit
      next(null, res);
    } else { // Cache miss
      // Extract sounding
      extractSounding(time, lat, lng, function(err, res) {
        if(err) {
          next(err);
        } else {
          var sounding = {time: time, loc: [lng, lat]};
          sounding.data = res;
          // Add to cache
          putCache(time, lat, lng, sounding, function(err) {
            if(err) {
              next(err);
            } else {
              // Return sounding
...
exports.preprocessSounding = function(fileName, next) {
  ...
  var time = new Date(parseInt(name.split('.')[0], 10));
  var newfile = fileName + '.small';
  var processor = spawn('grib2/wgrib2/wgrib2', [
    fileName,
    '-match',
    'HGT|TMP|UGRD|VGRD',
    '-match',
  ...
  processor.on('close', function(code) {
    ...
    if (code !== 0) {
      ...
      clearCache(time, function(err) {
        setFilename(time, newfile, function(err) {
          if(err) {
            next(err);
          } else {
            next(null, newfile);
