...
var IS = _.range(1, 361);
var JS = _.range(1, 182);
var HOURS = [
  0, 3, 6, 9, 12, 15, 18, 21,
...
var db = mongo.db('localhost:27017/stratocast');
...
function make_errorHandler(rl, next) {
...
function make_closeHandler(rl, next, parsed, lat, lng, timestamp) {
  return function(code) {
    ...
    if(code !== 0) {
      ...
      var sounding = {};
      sounding.time = timestamp;
      sounding.loc = [lng, lat];
      sounding.data = parsed;
      //db.collection('soundings').insert(sounding, function(err, res) {
      db.collection('soundings').update({time: timestamp, loc: [lng, lat]}, sounding, {upsert: true}, function(err, res) {
        if(err) {
          console.log("ERROR");
          setImmediate(function() {next(err);});
        }
        if(res) {
          console.log("Added " + timestamp + ' ' + lat + ', ' + lng);
...
function make_dataHandler(parsed) {
  var obs = {};
  return function(data) {
...
function make_parser(j, fileName) {
  var name = fileName.split('/')[2];
  var time = parseInt(name.split('.')[0], 10);
  ...
  return function(i, next) {
    ...
    parser.on('close', make_closeHandler(rl, next, parsed, -91 + j, i - 1, timestamp));
    rl.on('line', make_dataHandler(parsed));
  };
}

function make_parseRow(fileName) {
  return function(j, next) {
    async.eachSeries(IS, make_parser(j, fileName), function(err) {
      next(err);
    });
  };
}

exports.parseSounding = function(fileName, next) {
  async.eachSeries(JS, make_parseRow(fileName), function(err) {
    next(err);
  });
};
 ...
 * @param  {Function} next     Callback function
 */
exports.preprocess = function(fileName, next) {
